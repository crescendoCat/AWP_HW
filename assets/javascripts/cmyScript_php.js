...
function onPlayerReady(event) {
    ...
    // 取得第一段字幕的起訖時間
    var start = Number(caption_array.en[0].start);
    var duration = Number(caption_array.en[0].dur);
...
function onPlayerStateChange(event) {
    ...
    if (event.data == YT.PlayerState.PLAYING) {
       ...
       // console.log("PlayerState= PLAYING! CurrentTime:" + current);
        for (var i = 0; i < caption_array.en.length; i++) {
            start = Number(caption_array.en[i].start);
            duration = Number(caption_array.en[i].dur);
