    ...
    if (event.data == YT.PlayerState.PLAYING) {
       ...
       // console.log("PlayerState= PLAYING! CurrentTime:" + current);
        for (var i = 0; i < c.length; i++) {
            start = Number(c[i].start);
            duration = Number(c[i].dur);
...
function bindCaption() {
    ...
    for (var i = 0; i < c.length; i++) {
        start = Number(c[i].start);
        //console.log('caption: ' + c[i].text);
        end = start + Number(c[i].dur);
        captionTableContent = captionTableContent + ('<tr><td start=\'' + start + '\' end=\'' + end + '\'>\n<a href=\"javascript:;\" onclick=\"playVideo(' + start + ',' + end + ');\">\r<span>' + c[i].text + '</span></a>\n</td></tr>\r');
    }
